...
let errors = 0;
let bookingCounter = Date.now(); // Simple unique booking number generator
...
for (let i = 0; i < lines.length; i++) {
  ...
  for (let j = 2; j < Math.min(16, cols.length); j++) {
    ...
    const bookingDate = new Date(dates[dateIndex]);
    
    // Check if booking already exists
    const existing = await db.select().from(schema.bookings)
      .where(and(
        eq(schema.bookings.siteId, site.id),
        eq(schema.bookings.startDate, bookingDate),
        eq(schema.bookings.endDate, bookingDate)
      ));
    ...
    try {
      bookingCounter++;
      const totalAmount = parseFloat(site.pricePerDay) || 150;
      const gstAmount = totalAmount * 0.1;
      const platformFee = totalAmount * 0.05;
      const ownerAmount = totalAmount - gstAmount - platformFee;
      
      await db.insert(schema.bookings).values({
        bookingNumber: `TEST-${bookingCounter}`,
        siteId: site.id,
        customerId: 30001, // Owner user ID
      ...
      bookingsCreated++;
      console.log(`  ✅ Created booking: ${site.siteNumber}, ${dates[dateIndex]}`);
    } catch (error) {
      console.log(`  ❌ Error creating booking: ${site.siteNumber}, ${dates[dateIndex]}`, error.message);
      errors++;
    }
  }
}
