...
// Centre name to ID mapping
const centreMapping = {
  'Highlands Marketplace': 1,
  'Campbelltown Mall': 2,
  'Carnes Hill Marketplace': 3,
  'Pacific Square': 4,
  'Wanneroo Central': 5,
  'Rockdale Plaza': 6,
  'Bass Hill Plaza': 7,
...
for (let i = 0; i < lines.length; i++) {
  ...
  const siteResults = await db.select().from(schema.sites)
    .where(and(
      ...
      eq(schema.sites.siteNumber, siteNumber)
    ));
  
  if (siteResults.length === 0) {
    console.log(`  ⚠️  Site ${siteNumber} not found in ${currentCentre}`);
    errors++;
    continue;
  }
  ...
  for (let j = 2; j < Math.min(16, cols.length); j++) {
    ...
    if (existing.length > 0) {
      console.log(`  ⏭️  Booking already exists: Site ${siteNumber}, ${bookingDate}`);
      continue;
    }
