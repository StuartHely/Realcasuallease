...
export default function VacantShops() {
  ...
  const handleExport = async () => {
    ...
    try {
      const result = await utils.vsImportExport.exportVacantShops.fetch({
        centreId: selectedCentreId,
      });
      ...
      a.href = url;
      const centreName = selectedCentre?.name?.replace(/[^a-z0-9]/gi, "_") || "vacant_shops";
      a.download = `${centreName}_vacant_shops.csv`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success(`Exported ${result.count} vacant shops`);
    } catch (error: any) {
      ...
      <div className="space-y-6">
        ...
        <Dialog open={isImportOpen} onOpenChange={setIsImportOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Import Vacant Shops from Spreadsheet</DialogTitle>
              <DialogDescription>
                Upload a CSV file to create or update vacant shops. Existing shops (matched by Shop Number) will be updated.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                <p className="text-sm text-muted-foreground mb-2">
                  CSV columns: Shop Number, Total Size (m²), Dimensions, Powered Y/N, Description, Weekly Rate ($), Monthly Rate ($), Outgoings/Day ($), Active Y/N
                </p>
                <input
                  ref={fileInputRef}
                  type="file"
