 ...
 * 2. Stripe not configured → fall back to "invoice"
 * 3. stripe_with_exceptions + canPayByInvoice → "invoice"
 * 4. stripe → always "stripe" (ignores user flag)
 * 5. Otherwise → "stripe"
 */
export function resolvePaymentMethod(
  ...
  // Fall back to invoice if Stripe is not configured
  if (!ENV.stripeSecretKey) return "invoice";
